...
학생 <- read.csv("1_day/data/학생.csv", stringsAsFactors = FALSE)
학생2 <- readxl::read_xlsx("1_day/data/학생.xlsx", sheet = 1)
...
counties %>%
  select(men, women, hispanic, white, black)

# 코드 연습
# 수강생이 원하는 변수를 추출하세요. 
counties %>% 
  select(state, county, men) -> temp
...
counties_selected <- counties %>%
  ...
  summarise(minimum = min(population),
...
counties_selected <- counties %>%
  ...
  summarize(average_pop = mean(population),
            medium_pop = medium(population)) %>%
  filter(region == "North Central") %>%
  arrange(desc(arrange_pop)) -> final_df
