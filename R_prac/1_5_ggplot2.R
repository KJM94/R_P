...
ggplot(iris, 
       aes(x = Sepal.Length, 
           y = Sepal.Width)) + 
  geom_point(colour = "skyblue4") + # colour 지정
  theme_bw()

ggplot(mtcars, aes(x = cyl)) + 
  geom_bar(fill = "tomato3") # 모서리지점

# https://color.adobe.com/ko/create/color-wheel 컬러표
...
ggplot(iris, 
       aes(x = Sepal.Length, 
           y = Sepal.Width)) + 
  geom_point(colour = "skyblue4", size = 5)

# line의 두께
ggplot(economics, aes(x = date, y = uempmed)) + 
  geom_line(linetype = 7, size = 2)
...
p1 <- p + annotate(geom = "text", x = 6, y = 3, label = "setosa", size = 5) + 
  ggtitle("Using annoate() - 1") # annotate label의 위치를 x, y값으로 조정
